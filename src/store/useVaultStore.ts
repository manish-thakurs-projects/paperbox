...
const normalizeFolderIds = (folderIds: string[] = []) =>
 folderIds.filter((folderId, index, list) => folderId && list.indexOf(folderId) === index);
...
export const useVaultStore = create<State>((set, get) => ({
 ...
 toggleFavorite: (id) => {
   const files = normalizeFiles(
     get().files.map((f) => (f.id === id ? { ...f, isFavorite: !f.isFavorite } : f)),
   );
   set({ files });
   persist(files, get().folders);
 },
 togglePin: (id) => {
   const files = normalizeFiles(
     get().files.map((f) => (f.id === id ? { ...f, isPinned: !f.isPinned } : f)),
   );
   set({ files });
   persist(files, get().folders);
 },
     ...
     get().files.map((f) =>
       f.id === id
         ? {
             ...
             folderIds: normalizedFolderIds.length ? normalizedFolderIds : undefined,
           }
         : f,
     ),
   );
